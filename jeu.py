        ...
        while self.run:
            ...
            for event in pygame.event.get():
                ...
                # Touche pressée
                    ...
                    if self.nom_actif and ecran1.ecran.get_actif():
                    ...
                    if ecran_mort.ecran.get_actif():
                        ...
                        if self.nb_cb_actif:
                            if event.key == pygame.K_BACKSPACE:
                                self.nbr_cb = self.nbr_cb[:-1]
                            elif len(self.nbr_cb) < 19 and event.unicode in "123456789":
                                L = [4,9,14]
                                for elem in L:
                                    if len(self.nbr_cb) == elem:
                                        self.nbr_cb += ' '
                                self.nbr_cb += event.unicode

                        elif self.code_cb_actif:
                            # Gérer la saisie du code de carte bleue
                            if event.key == pygame.K_RETURN:
                                if len(self.nbr_cb) == 19 and len(self.codee_cb) == 4:
                                    self.nbr_cb = ''
                                    self.codee_cb = ''
                                ...
                                self.codee_cb = self.codee_cb[:-1]
                            elif len(self.codee_cb) < 4 and event.unicode in "123456789":
                                self.codee_cb += event.unicode
                    
                    if ecran_machine_a_sous.ecran.get_actif():
